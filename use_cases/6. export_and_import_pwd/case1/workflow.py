...
def invert(image):
    in_im = im2im(image, 'numpy.gray_float64(0to1)')
    image = IM(util.invert(in_im.raw_image), in_im.metadata)
    return image
...
def save_image(image, name, destination):
    in_im = im2im(image, 'numpy.uint8')
    io.imsave(join(destination, name), in_im.raw_image)



def save_to_csv(data, name, destination):
    pd.DataFrame(data).to_csv(join(destination, name), index=False)
